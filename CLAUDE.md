...
**The site is mid-migration between two themes.** The homepage uses the "instrument bench" design (July 2026): a dark engineering-lab identity — near-black blue-gray panels on a faint graticule grid, oscilloscope channel colors as the accent system (CH1 amber `--ch1: #f2b63d`, CH2 cyan `--ch2: #4fc3dc`, CH3 magenta `--ch3: #d76bd0`), code-comment eyebrows (`// identification`), a typed-name hero with amber block cursor, and an "AT-87 Scopemeter" panel (`scope.js`) that live-decodes an I²C transaction and connects to the three nav cards via colored probe leads. Fonts: Chakra Petch (display caps), IBM Plex Sans (body), IBM Plex Mono (labels/readouts). The other three pages still use the older dark-violet theme (`--violet-500: #9747ff`, Space Grotesk/Cormorant Garamond/Fira Code) and are pending rollover. An earlier warm-paper "cartographic survey" homepage was built and rejected — the user wants tech-flavored aesthetics, with the topographic contours kept only as a background visualization.
...
- `scope.js` — homepage-only instrument graphics: renders an animated I²C transaction (CH1 = SCL clock, CH2 = SDA data, transmitting "ARYAN THOMARE" in ASCII with logic-analyzer decode labels) on `#scope-canvas`, and draws the probe leads from the meter's jacks to the three index cards on `#probe-canvas` (positions measured from the live DOM; skipped when cards aren't on one row). Colors from `--ch1`/`--ch2`/`--ch3`. Static frame under reduced motion.
